...
export const ShaderCanvas = ({ 
  ...
  useEffect(() => {
    ...
    console.log("Initializing WebGL with size:", size, "canvas:", canvas);
    ...
    const render = () => {
      ...
      try {
        ...
        console.error("Error in drawScene:", error);
        setWebglError(true);
        return;
      }
  ...
  if (webglError) {
    return (
      <div 
        className="rounded-full transition-transform duration-300 flex items-center justify-center" 
        style={{ 
          width: size, 
          height: size,
          ...
          background: hasActiveReminders 
            ? 'radial-gradient(circle, rgba(66, 153, 225, 0.3) 0%, rgba(66, 153, 225, 0.1) 50%, transparent 100%)' 
            : hasUpcomingReminders
              ? 'radial-gradient(circle, rgba(72, 187, 120, 0.3) 0%, rgba(72, 187, 120, 0.1) 50%, transparent 100%)'
              : 'radial-gradient(circle, rgba(255, 255, 255, 0.2) 0%, rgba(255, 255, 255, 0.05) 50%, transparent 100%)',
          boxShadow: hasActiveReminders 
            ? '0 0 50px rgba(66, 153, 225, 0.6)' 
            : hasUpcomingReminders
              ? '0 0 50px rgba(72, 187, 120, 0.6)'
      ...
      >
        <div className="text-white/50 text-center">
          ...
          <div className="text-sm">WebGL Fallback</div>
        </div>
      </div>
    );
  }
